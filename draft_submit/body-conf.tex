...
To evaluate the architecture represented by the CGP, we train the network using training dataset in an ordinary way. Then the performance for another validation dataset is assigned as the \new{fitness} of the architecture. Based on this fitness evaluation, an evolutionary algorithm optimizes the CNN architectures.
 To check the performance of the proposed \new{method}, we conducted the experiment constructing the CNN architecture for the image classification task with the CIFAR-10 dataset \cite{krizhevsky_learning_2009}. The experimental result shows that the proposed \new{method} can automatically find the competitive CNN architecture compared with state-of-the-art models.
...
The hyperparameter optimization approach often tunes the predefined hyperparameters such as the number\new{s} of layers and neurons, and the type of activation functions. While this method has seen success, it is hard to design more flexible architectures from scratch.
...
\shin{I will modify the explanation of the Verbancsics's papers.}
...
\caption{Example of a genotype and a phenotype. The genotype (Left) defines \new{the} CNN architecture (Right). \new{The max-pooling operation is inserted before the summation processing because each input feature map to the summation node is a different size.}}
\label{genotype}
\end{figure*}

% ----------------------------------------------------------------------------------------------------
\section{CNN Architecture Design Using Cartesian Genetic Programming}
Our method directly encodes the CNN architectures based on CGP \new{\cite{harding_evolution_2008,Miller2006,miller_cartesian_2000}} and uses the highly-functional modules as the node functions.
...
We prepare the six types of node functions called ConvBlock, \new{ResBlock,} max-pooling, average-pooling, tensor concatenation, and tensor summation.

The ConvBlock consists of a standard convolution processing \new{with stride 1} followed by batch normalization \cite{ioffe_batch_2015} and rectified linear units (ReLU) \cite{nair_rectified_2010}. In the ConvBlock, we pad the outside of \new{an} input feature map with zero values before the convolution operation so as to keep the \new{row and column sizes} of the output. 
As a result, the $M \times N \times C$ input feature map is transformed into the \new{$M \times N \times C'$ output, where $M$, $N$, $C$, and $C'$ are the numbers of rows, columns, input channels, and output channels, \del{of a convolution layer} respectively.}

The ResBlock is composed of a convolution processing, batch normalization, ReLU, and \new{tensor summation}. A ResBlock architecture is shown in Figure \ref{resblock}.
The ResBlock performs identity mapping by shortcut connections \new{as} described in \cite{he_deep_2016}.
\new{The row and column sizes of the input are} preserved in the same way as ConvBlock after convolution.
\new{The} output feature map of the ResBlock is calculated by \new{the} ReLU \new{activation} and \new{the tensor summation with the} input feature map and \new{the processed feature map as shown} in Figure \ref{resblock}.
In the ResBlock, the $M \times N \times C$ input feature map is transformed into the $M\times N\times C'$ output.

The max- and average-pooling\new{s} use the maximum and average values as the values of the down-sampled images, respectively.
In the pooling operation, the $M \times N \times C$ input feature map is transformed into the $M' \times N' \times C$ output, where \new{$M' = \lfloor M/2 \rfloor$ and $N' = \lfloor N/2 \rfloor$}.

The tensor concatenation concatenates two feature maps in the \new{channel} dimensions.
If the input feature maps to be concatenated have the different number of rows or column\new{s}, we down-sample the larger feature map by max-pooling so as to become the same sizes of inputs.
...
Moreover, to keep a neutral drift, which is effective for CGP evolution \new{\cite{Miller2006,miller_cartesian_2000}}, we modify a parent by the neutral mutation if the fitnesses of the offsprings do not improve.
...
In the default scenario, we randomly sampled $45,000$ images from the training set to train the CNN and use the remaining $5,000$ images for the validation set \new{of the} CGP fitness evaluation.
...
\new{We train each CNN for 50 epochs and reduce the learning rate by a factor of 10 at 30th epoch.}

We preprocess the data with the per-pixel mean subtraction.
To prevent overfitting, we use weight decay with \new{the} coefficient $1.0\times 10^{-4}$ and data augmentation.
We use the data augmentation method based on \new{\cite{he_deep_2016}}; padding $4$ pixels on each side followed by choosing a random $32\times 32$ crop from the padded image, and random horizontal flips on the cropped $32 \times 32$ image.

\new{We implement our methods using the Chainer \cite{tokui_chainer:_2015} (version 1.16.0) framework and run the experiments on the machines with $3.2$GHz CPU, $32$GB RAM, and two NVIDIA GeForce GTX 1080 GPUs.}
...
\new{The offspring size $\lambda$ is set to two that is the same number of GPUs in our experimental machines.
The numbers of generations are $300$ and $2,000$ for the default and small-data scenario, respectively.} \shin{It may change...}

\todo{Describe about node function sets; ConvSet, ResSet}
